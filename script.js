...
function generateQuestions() {
    const generators = [
        ...
        () => {
            ...
            return { q, a: simplifyFraction(ansN, d) };
        },
        // 2. 分数の加減（異分母 - 簡単な倍数関係）
        () => {
            const d1 = [2, 3, 5][Math.floor(Math.random() * 3)];
            const d2 = d1 * (Math.floor(Math.random() * 3) + 2); // 2, 3, 4倍
            const n1 = 1;
            const n2 = 1;
            const op = Math.random() > 0.5 ? '+' : '-';
            let q, ansN, ansD;
            ...
            return { q, a: simplifyFraction(ansN, ansD) };
        },
        // 3. 分数 × 整数
        () => {
            ...
            return {
                q: `${n}/${d} × ${mult} = ?`,
                a: simplifyFraction(n * mult, d)
            };
        },
        // 4. 小数 × 整数
        () => {
            const val = (Math.floor(Math.random() * 19) + 1) / 10; // 0.1 - 1.9
            const mult = Math.floor(Math.random() * 9) + 2; // 2-10
            const ans = parseFloat((val * mult).toFixed(2));
            return {
                q: `${val} × ${mult} = ?`,
                a: `${ans}`
            };
        ...
        // 5. 小数 × 小数
        () => {
            const v1 = (Math.floor(Math.random() * 9) + 1) / 10; // 0.1 - 0.9
            const v2 = (Math.floor(Math.random() * 9) + 1) / 10; // 0.1 - 0.9
            const ans = parseFloat((v1 * v2).toFixed(2));
            return {
                q: `${v1} × ${v2} = ?`,
                a: `${ans}`
            };
        ...
        () => {
            ...
            return Math.random() > 0.5
                ? { q: `${item.f} を小数にすると？`, a: item.d }
                : { q: `${item.d} を分数にすると？`, a: item.f };
        }
    ];

    const qs = [];
    for (let i = 0; i < 10; i++) {
        // 各タイプから重複を抑えつつ選ぶ（タイプをランダムに選ぶ）
        const gen = generators[Math.floor(Math.random() * generators.length)];
        let item = gen();

        // 既に同じ問題があれば作り直し（最大5回トライ）
        let retry = 0;
        while (qs.some(existing => existing.q === item.q) && retry < 5) {
            item = gen();
            retry++;
        }

        // 選択肢の生成
        const options = new Set([item.a]);
        let failSafe = 0;
        while (options.size < 4 && failSafe < 20) {
            const randomTypeGen = generators[Math.floor(Math.random() * generators.length)];
            const fake = randomTypeGen().a;
            if (fake !== item.a) options.add(fake);
            failSafe++;
        }
        // 足りない場合のフォールバック
        if (options.size < 4) {
...
startBtn.onclick = () => {
    if (!bgmPlaying) {
        ...
        initBGM();
        if (bgm) bgm.play().catch(e => console.log("BGM start play failed:", e));
    }
    playSFX('ora_ora.mp3');
    startGame();
};
retryBtn.onclick = () => {
    ...
    if (bgmPlaying) {
        initBGM();
        if (bgm) bgm.play().catch(e => console.log("BGM retry play failed:", e));
    }
    playSFX('ora_ora.mp3');
    startGame();
};
